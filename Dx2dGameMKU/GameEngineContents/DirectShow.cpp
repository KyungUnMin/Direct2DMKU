...
BOOL CDirectShow::PlayClip(HWND hWnd, char* szFile)
{
	if( m_bReady == TRUE )
	{
		Release();
	...
	if( FAILED(hResult) )
	{
		MsgAssert("다이렉트쇼우 IID_IGraphBuilder 생성에 실패했습니다");
		return FALSE;
	}
	...
	if( SUCCEEDED(hResult) )
	{
		...
		//지원되지 않는 형식
		if (hResult == VFW_E_UNSUPPORTED_STREAM)
		{
			int a = hResult;
		}
		...
		if( FAILED(hResult) )
		{
			MsgAssert("다이렉트쇼우 RenderFile에 실패했습니다(CODE이 없습니다");
			return FALSE;
		}
...
BOOL CDirectShow::IS_FINISHED()
{
	...
	if( m_pMS )
	{
		...
		if( FAILED(hResult) )
		{
			MsgAssert("IS_FINISHED에서 m_pMS->GetStopPosition함수를 실행하는데 실패했습니다");
			return FALSE;
		}
